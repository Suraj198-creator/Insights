...
ai_emails = supabase.table("emails").select("email").in_("id", ai_email_ids).execute()
crypto_emails = supabase.table("emails").select("email").in_("id", crypto_email_ids).execute()

with open("ai_subscribers.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["email"])
    for e in ai_emails.data:
        writer.writerow([e["email"]])
ai_response = ai_emails.data


with open("crypto_subscribers.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["email"])
